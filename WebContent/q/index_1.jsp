...
{
	// load device
	Q					deviceQ = Factory.getServices().getDeviceManager().loadDevice(device);
	...
	if ( Factory.getServices().getqManager().isValidQ(qid) )
	{
		...
		/*
		for ( Q qq : (List<Q>)Database.getSession().createCriteria(Q.class)
				.add(Restrictions.eq("q", qid))
				.add(Restrictions.eq("dataType", "post"))
	...
	if ( request.getParameter("position") != null )
	{
		...
		if ( request.getParameter("latitude") != null )
		{
			...
			deviceQ.setLatitude(Double.parseDouble(request.getParameter("latitude")));
		}
		if ( request.getParameter("longitude") != null )
		{
			q.setLongitude(Double.parseDouble(request.getParameter("longitude")));
			deviceQ.setLongitude(Double.parseDouble(request.getParameter("longitude")));
		}
		if ( request.getParameter("altitude") != null )
		{
			q.setAltitude(Double.parseDouble(request.getParameter("altitude")));
			deviceQ.setAltitude(Double.parseDouble(request.getParameter("altitude")));
		}
...
<% } %>
		...
		<script src="<%=cdnUrl%>js/vendor/canvasresize-1.2.0/binaryajax.js"></script>
        <script src="<%=cdnUrl%>js/vendor/canvasresize-1.2.0/exif.js"></script>
        <script src="<%=cdnUrl%>js/vendor/canvasresize-1.2.0/canvasResize.js"></script>
		...
		function form_submit()
		{
			...
			try
			{
				if ( file_resized_data != null )
				{
					file = $("#file")[0];
					
                    var 	fd = new FormData();
                    var 	f = canvasResize('dataURLtoBlob', file_resized_data);
                    f.name = file.name;
                    fd.append("file", f);
					...
					for ( var n = 0 ; n < data.length ; n++ )
					{
						...
						if ( field.name == "upload" )
							fd.append(field.name, "3");
						else if ( field.name != "file" )
							fd.append(field.name, field.value);
					}
                    
                    var xhr = new XMLHttpRequest();
                    xhr.open('POST', '<%=qid%>', true);
...
{
	log.debug("popping session");
	sessionManager.popThreadSession();
	log.debug("session popped");
}
log.debug("done");
%>
